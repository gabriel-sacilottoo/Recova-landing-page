...
export function PlanFeature({ label, active }: { label: string; active: boolean }) {
  return (
    <li className="flex items-center gap-[6px]">
      <MeaningfulIcon name="check" size={20} className={`size-5 shrink-0 ${active ? "text-accent" : "text-ink-200"}`} />
      <span
...
export function PlanChip({ label, icon }: { label: string; icon: string }) {
  return (
    <span className="flex shrink-0 items-center gap-1 rounded-pill bg-surface py-1 pr-[10px] pl-2">
      <MeaningfulIcon name={label || icon} size={20} className="size-5 text-accent" />
...
export function PlanCta({ plan }: { plan: PricingPlan }) {
  return (
    ...
    >
      {plan.ctaLabel}
      <MeaningfulIcon name="arrow" size={24} className="size-6" />
    </Link>
  );
}
...
export function PricingCard({ plan, featured = false }: { plan: PricingPlan; featured?: boolean }) {
  return (
    <article
      className={`relative flex w-full max-w-[435px] flex-col items-start gap-[10px] overflow-hidden rounded-t-panel hairline rounded-b-[24px] p-[14px] transition-all duration-300 hover:-translate-y-1 ${
        featured
          ? "border-2 border-[#065cf5] shadow-[0_24px_50px_-12px_rgba(6,92,245,0.35)]"
          : "shadow-[var(--shadow-ring)] hover:border-[#065cf5]/40 hover:shadow-[0_20px_40px_-12px_rgba(6,53,93,0.2)]"
      }`}
    >
      {featured && (
        <span className="absolute top-0 left-1/2 z-20 -translate-x-1/2 rounded-b-pill bg-[#065cf5] px-4 py-1.5 text-[12px] font-bold tracking-wide text-white uppercase shadow-[0_6px_16px_-4px_rgba(6,92,245,0.5)]">
          Mais popular
        </span>
      )}
      <Image
