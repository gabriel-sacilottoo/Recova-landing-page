...
function TestimonialCard({ item }: { item: Testimonial }) {
  return (
    <figure className="relative flex w-full flex-col gap-5 overflow-hidden rounded-panel border border-line p-6 shadow-[var(--shadow-ring)]">
      ...
      />
      <span className="relative flex items-center gap-0.5 text-amber-400" aria-label="Avaliação positiva">
        {[0, 1, 2, 3, 4].map((star) => (
          <MeaningfulIcon key={star} name="star" size={18} className="size-[18px] fill-current" />
        ))}
      </span>
    ...
    >
      <div className="relative flex w-full flex-col items-center gap-4">
        ...
        >
          ...
          <div className="absolute top-[93px] left-1/2 z-[1] flex size-[112px] -translate-x-1/2 items-center rounded-pill bg-[rgba(6,92,245,0.2)] p-[14px]">
            <div className="relative flex size-[84px] items-center justify-center overflow-hidden rounded-pill p-[18px] shadow-[0_7px_9.3px_0_rgba(131,124,124,0.06),inset_0_2.3px_2.3px_0_#81b1fb,inset_2.3px_0_2.3px_0_#81b1fb]">
              ...
              />
              <span className="relative flex size-[47px] items-center justify-center rounded-pill bg-[#065cf5] text-[22px] font-bold text-white">
                R
              </span>
            </div>
          </div>
